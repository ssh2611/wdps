...
def process_page(row):
    ...
    yield ("%s ---------tttt  %s tttt --------- %s - - -- - - -" % (row, warc_record.id, warc_payload))
    ...
    #soup = None
    #try:
    #    soup = BeautifulSoup(warc_payload)
    #except:
...
if __name__ == '__main__':
    ...
    input_lines  = sc.textFile("sample.warc.gz").collect()[0:1000]
    raw_records = sc.parallelize(split_records())
    rdd = raw_records.flatMap(process_page)
    rdd = rdd.saveAsTextFile("sample_predications_tmp97")
